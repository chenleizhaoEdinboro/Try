...
public class EdgeWeightedGraph {
    ...
    private final int V; //total vertex
    private int E;       //total edges
   ...
   /* public EdgeWeightedGraph(int V, int E) {
	...
	public EdgeWeightedGraph(String fileName) throws IOException{
		...
		int E = s.nextInt(); //16
		for (int i = 0; i < E; i++) {
            int vertex = s.nextInt();//4
            int w = s.nextInt();//5
            double weight = s.nextDouble();//.35
	...
	public EdgeWeightedGraph(String fileName) throws IOException{
		...
		//int E = s.nextInt(); //16
		   while(s.hasNextInt()){
			   ...
			   double weightCost = s.nextDouble();
               Edge e = new Edge(vertex,w,weightDist,weightCost,placeName);
               addEdge(e);
           }
    ...
    public void addEdge(Edge e) {
        ...
        validateVertex(v);
        validateVertex(w);
        adj[v].add(e);//add one edge twice for both ends of vertex
        adj[w].add(e);
        E++;
    }
    ...
    public Iterable<Edge> edges() {
        ...
        for (int v = 1; v <= V; v++) {
            int selfLoops = 0;
			System.out.println("the adj(v) is "+v);
            for (Edge e : adj(v)) {
                if (e.other(v) > v) {
                    list.add(e);
                }
    ...
    public String toString() {
        ...
        s.append("total vertice"+V +" total edges "+E + NEWLINE);
        for (int v = 1; v <=V; v++) {
            s.append(v + ": ");
            for (Edge e : adj[v]) {
        ...
        return s.toString(); //actually calling the toString in the Edge
    }
