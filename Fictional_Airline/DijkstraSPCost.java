...
public class DijkstraSPCost {
     ...
     */
    public DijkstraSPCost(EdgeWeightedDigraph G, String start, String end) {
		
		int s = G.getPlaceNumber(start);
		int t = G.getPlaceNumber(end);
		if(s == -3 || t==-3){
			System.out.println("Check your spelling, the place is not found");
		}
        ...
        while (!pq.isEmpty()) {
			 ...
			 if(v == t)
				 break;
			//check every edge then do the relaxation
    ...
    private void relax(DirectedEdge e) {
        ...
        if (distTo[w] > distTo[v] + e.weightCost()) {
            distTo[w] = distTo[v] + e.weightCost();
            edgeTo[w] = e;
    ...
    private boolean check(EdgeWeightedDigraph G, int s) {
        ...
        for (DirectedEdge e : G.edges()) {
            if (e.weightCost() < 0) {
                System.err.println("negative edge weight cost detected");
                return false;
            }
        }
    ...
    public static void main(String[] args) throws IOException{
       ...
       // int s = Integer.parseInt(args[0]); //start the vertex 0
        ...
        DijkstraSPCost sp = new DijkstraSPCost(G, startPlace,endPlace);


        // print shortest path
        //for (int t = 1; t <= G.V(); t++) {
			int t = G.getPlaceNumber(endPlace);
            if (sp.hasPathTo(t)) {
                StdOut.printf("%s to %d (%.2f)  ", startPlace, endPlace, sp.distTo(t));
