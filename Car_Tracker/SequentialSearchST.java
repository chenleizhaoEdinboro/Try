...
public class SequentialSearchST<Key extends Comparable<Key>,Value extends Comparable<Value>>{
	...
	public boolean containsN(int N){
		return getN(N) != null;
	}
    /**
     ...
     */
    public Value get(String vin) {
        for (Node x = first; x != null; x = x.next) {
            if (vin.equals(x.val.getVinNum()))
                return x.val;
        }
        return null;
    }

	public Value getN(int n){
     ...
     * @param val the value
     */
    public void put(String vin, Value val) {
        if (val == null) {
            delete(vin);
            return;
        }
         //x is the temp variable
        for (Node x = first; x != null; x = x.next) {
            if (vin.equals(x.getVinNum())) {
                x.val = val;
                return;
            }
        }
		//Starting to creat the separate chain
        first = new Node(vin, val, first);
        N++;
    }
